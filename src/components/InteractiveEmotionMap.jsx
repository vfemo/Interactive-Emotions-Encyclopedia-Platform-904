...
const InteractiveEmotionMap = ({ emotions, hoveredEmotion, setHoveredEmotion }) => {
  const { theme } = useTheme();
  const emotionArray = Object.values(emotions);

  // Create a tree-like layout for emotions
  ...
  return (
    <div className={`relative w-full h-96 ${theme.colors.card} rounded-3xl overflow-hidden shadow-2xl`}>
      {/* Background Pattern */}
      ...
      >
        <div className={`w-20 h-20 rounded-full bg-gradient-to-r ${theme.colors.primary} flex items-center justify-center shadow-lg`}>
        ...
        return (
          ...
          >
            <Link to={`/emocja/${emotion.id}`}>
              ...
              {/* Emotion Label */}
              <motion.div
                className={`absolute top-20 left-1/2 transform -translate-x-1/2 ${theme.colors.card} px-3 py-1 rounded-full text-sm font-medium ${theme.colors.text} shadow-lg whitespace-nowrap`}
      ...
      {hoveredEmotion && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 20 }}
          className={`absolute bottom-4 left-4 right-4 ${theme.colors.card} rounded-xl p-4 shadow-xl border border-white/20`}
        >
...
export default InteractiveEmotionMap;